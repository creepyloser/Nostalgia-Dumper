...
namespace GUI
{
    ...
    bool Initialize()
    {
        ...
        g_hWnd = ::CreateWindowExW(0, g_wc.lpszClassName, L"Nostalgia", WS_OVERLAPPEDWINDOW, 100, 100,
    ...
    void RequestHeaderGeneration(AppState& appState)
    {
        ...
        const std::string content = Dumper::GenerateHeaderContent(appState.foundOffsets, gameName, extras);
    ...
    bool RenderFrame(AppState& appState)
    {
        ...
        if (appState.shouldMinimize && g_hWnd) {
            ::ShowWindow(g_hWnd, SW_MINIMIZE);
            appState.shouldMinimize = false;
        }
        ...
        ImGui::TextColored(ImVec4(0.12f, 0.45f, 1.00f, 1.00f), "NOSTALGIA");
        ImGui::SameLine();
        ImGui::TextDisabled("| v%s", NOSTALGIA_VERSION_STR);

        // Window controls
        float windowWidth = ImGui::GetWindowWidth();
        ImGui::SetCursorPos(ImVec2(windowWidth - 70, 5));
        if (ImGui::Button("_", ImVec2(25, 25))) appState.shouldMinimize = true;
        ImGui::SameLine();
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.8f, 0.1f, 0.1f, 1.0f));
        if (ImGui::Button("X", ImVec2(25, 25))) appState.shouldClose = true;
        ImGui::PopStyleColor();
        ...
        if (ImGui::BeginTabBar("NostalgiaTabs", ImGuiTabBarFlags_None))
        {
            if (ImGui::BeginTabItem("DASHBOARD"))
            {
                ...
                ImGui::SetNextItemWidth(-1);
                ImGui::InputTextWithHint("##Path", "C:\\Users\\...", appState.outputPathBuffer, sizeof(appState.outputPathBuffer));
                ImGui::Spacing();
                ImGui::BeginDisabled(!appState.scanComplete);
                if (ImGui::Button("GENERATE OFFSET HEADER", ImVec2(-1, 35)))
            ...
            if (ImGui::BeginTabItem("DISCOVERED DATA"))
            {
                if (appState.scanComplete) {
                    ...
                    if (ImGui::BeginTable("DataTable", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable | ImGuiTableFlags_ScrollY)) {
                        ImGui::TableSetupColumn("Identifier");
                        ImGui::TableSetupColumn("Address", ImGuiTableColumnFlags_WidthFixed, 140.0f);
                        ImGui::TableSetupColumn("Action", ImGuiTableColumnFlags_WidthFixed, 80.0f);
                        ImGui::TableHeadersRow();

                        auto addRow = [&](const char* name, u64 addr) {
                            if (appState.globalFilter[0] != '\0') {
                                ...
                                if (n.find(f) == std::string::npos) return;
                            }
                            ImGui::TableNextRow();
                            ImGui::TableSetColumnIndex(0); ImGui::TextUnformatted(name);
                            ImGui::TableSetColumnIndex(1); if (addr) ImGui::Text("0x%llX", addr); else ImGui::TextDisabled("Not Found");
                            ImGui::TableSetColumnIndex(2); if (addr) {
                                if (ImGui::SmallButton("Copy")) {
                                    char hex[32]; sprintf(hex, "0x%llX", addr);
                                    ImGui::SetClipboardText(hex);
                                }
                            }
                        };

                        addRow("GObjects", appState.foundOffsets.GObjects.address);
                        addRow("GNames", appState.foundOffsets.GNames.address);
                        addRow("GWorld", appState.foundOffsets.GWorld.address);
                        addRow("GEngine", appState.foundOffsets.GEngine.address);
                        for (const auto& [name, addr] : appState.engineSymbols) addRow(name.c_str(), addr);
                        ImGui::EndTable();
                    }
                } else {
        ...
        if (ImGui::BeginPopupModal("Changelog", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            ImGui::Text("Welcome %s to the Nostalgia beta!", pcName);
            ImGui::Text("Please send a review to me, or star the GitHub Repository!");
            ImGui::Separator();
            ImGui::Text("Nostalgia v1.8.0 Omni-Engine Update");
            ImGui::Separator();
            ImGui::Text("- Added GameMaker Studio support");
            ImGui::Text("- Added UNIGINE support");
            ImGui::Text("- Added Personalized Welcome Message");
            ImGui::Text("- Refined engine-wide discovery for peak performance");
            ImGui::Text("- Improved UI with new engine options");
            ImGui::Spacing();
