...
namespace Dumper
{
    ...
    /** JSON: globals with signatures/modules, custom offsets, script path hints, UObject samples. */
    std::string GenerateFullDumpJson(const DiscoveredOffsets& offsets, const std::string& gameName = "Game",
                                     const std::map<std::string, u64>* extraSymbols = nullptr);
